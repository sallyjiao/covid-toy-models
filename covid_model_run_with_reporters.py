...
obj = covid_model.COVID_Model(500, 0.25, 0.25, 
                              dt=1.0, 
                              init_protocol = 0, 
                              asymptomatic_rate=asymptomatic_rate,
                              reporter_names=['incidence'])
obj.step( assign_work = obj.assign_work_random, quarantine=obj.quarantine_lab_shutdown, fraction_working=fraction_working, lab_shutdown_max=lab_shutdown_max)

# Run model for 201 more days:
covid_model.verbosity = 0
for iters in range(200+1):
    time,history = obj.step( assign_work = obj.assign_work_random, quarantine=obj.quarantine_lab_shutdown, fraction_working=fraction_working, lab_shutdown_max=lab_shutdown_max )
...
for rep_name in obj.reporters.keys():
    print(obj.reporters[rep_name])
